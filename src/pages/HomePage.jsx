...
function VapiButton({ triggerRef }) {
  const [status, setStatus]   = useState("idle")
  const [volume, setVolume]   = useState(0)
  const [showTip, setShowTip] = useState(false)
  const tipTimer = useRef(null)

  // Expose a start method so HomestayList's Customer Support button can trigger it
  useEffect(() => {
  ...
  const handleClick = async () => {
    const vapi = getVapi()
    setShowTip(false)
    if (status === "active")     { vapi.stop(); return }
  ...
  const bgColor = status === "active"
    ? "#ef4444"
    : status === "connecting" ? "#8B6914"
    : status === "error"      ? "#ef4444"
    : "#2D5A3D"

  const scale = status === "active" ? 1 + volume * 0.3 : 1

  const label = status === "active"      ? "Tap to end call"
  ...
  return (
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ position: "relative", width: 48, height: 48 }}>
        ...
        <button onClick={handleClick} title={label} style={{
...
export default function HomePage({ onLogoClick }) {
  ...
  const vapiTriggerRef = useRef(null) // ← ref to call handleClick on VapiButton externally
  ...
  // Called by HomestayList's "Customer Support" button
  const handleSupportClick = () => {
    if (vapiTriggerRef.current) vapiTriggerRef.current()
  }
  ...
  return (
    <div className="soul-bg min-h-screen bg-gradient-to-b from-[#1C1C1C] via-[#2C2C2C] to-[#1a1f1a]">
      ...
      <div style={{
        ...
        <div style={{
          width: phase === 0 ? "80px" : "0px", height: "1px",
      ...
      {/* Fixed bottom-right: Vapi + scroll buttons */}
      <div className="fixed right-5 bottom-8 flex flex-col items-center gap-2 z-50">
        <VapiButton triggerRef={vapiTriggerRef} />
